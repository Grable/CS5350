...
class NN:
    def __init__(self, width):
        ...
        for i in range(1, self.layers-1):
           wi = np.random.normal(0, 1, (self.width[i] - 1, self.width[i-1])) # this one uses the normal weight vector
        
        
        #  wi = np.zeros([self.width[i] - 1, self.width[i-1]]) #this one uses the 0 vector
